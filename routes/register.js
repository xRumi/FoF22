...
router.post('/confirm/post', async (req, res) => {
    ...
    if (_token && username?.length >= 4 && username.length <= 16 && name?.length >= 4 && name.length <= 32 && password?.length >= 8 && password.length <= 32 && ObjectID.isValid(_token)) {
        if (!username.match(/^[^a-zA-Z0-9]+$/)) return res.status(400).send('Special characters in username is not allowed');
        if (!name.match(/^[^a-zA-Z0-9]+$/)) return res.status(400).send('Special characters in full name is not allowed');
